...
#ifndef _RF24_INTERFACE_H__
#define _RF24_INTERFACE_H__
...
#ifdef CONFIG_NRF24_I2C
#include "i2c_bus.h"
typedef struct i2c rf24_bus_t;
typedef rf24_i2caddr_t rf24_addr_handle_t;
#elif CONFIG_NRF24_SPI
#include "spi_bus.h"
typedef struct spi rf24_bus_t;
typedef spi_device_handle_t rf24_addr_handle_t;
#endif
 ...
 */
struct rf24 {
    rf24_bus_t *bus;            //!< I2C(SPI) device descriptor
    rf24_addr_handle_t addr;
     ...
     *
     */
    void (*beginTransaction)(struct rf24 *nrf24);
    void (*endTransaction)(struct rf24 *nrf24);
     ...
     */
    uint8_t (*read_register)(struct rf24 *nrf24, uint8_t reg);
     ...
     */
    WK_RESULT (*read_registers)(struct rf24 *nrf24, uint8_t reg, uint8_t* buf, uint8_t len);
     ...
     * byte, but that it now saved to a private member on all SPI transactions.
     */
    WK_RESULT (*write_register)(struct rf24 *nrf24, uint8_t reg, uint8_t value, bool is_cmd_only);
     ...
     * byte, but that it now saved to a private member on all SPI transactions.
     */
    WK_RESULT (*write_registers)(struct rf24 *nrf24, uint8_t reg, const uint8_t* buf, uint8_t len);
     ...
     * previously called.
     */
    WK_RESULT (*_init_radio)();

    /**
     * initialize the GPIO pins
     */
    WK_RESULT (*_init_pins)();
     ...
     * @endcode
     * @return
     * - `WK_OK` if the radio was successfully initialized
     * - `WK_FAIL` if the MCU failed to communicate with the radio hardware
     */
    WK_RESULT (*begin)(struct rf24 *nrf24);
     ...
     */
    WK_RESULT (*csn)(struct rf24 *nrf24, bool mode);
     ...
     */
    WK_RESULT (*ce)(struct rf24 *nrf24, bool level);
     ...
     * byte, but that it now saved to a private member on all SPI transactions.
     */
    WK_RESULT (*write_payload)(struct rf24 *nrf24, const void* buf, uint8_t len, const uint8_t writeType);
     ...
     * byte, but that it now saved to a private member on all SPI transactions.
     */
    WK_RESULT (*read_payload)(struct rf24 *nrf24, void* buf, uint8_t len);

    /**
     * Checks if the chip is connected to the SPI bus
     */
    bool (*isChipConnected)(struct rf24 *nrf24);
     ...
     * functionality.
     */
    WK_RESULT (*startListening)(struct rf24 *nrf24);
     ...
     */
    WK_RESULT (*stopListening)(struct rf24 *nrf24);
     ...
     * @endcode
     */
    bool (*isAvailable)(struct rf24 *nrf24);
     ...
     * @endcode
     */
    WK_RESULT (*read)(struct rf24 *nrf24, void* buf, uint8_t len);
     ...
     *   is on.
     */
    WK_RESULT (*write)(struct rf24 *nrf24, const void* buf, uint8_t len);
    ...
    WK_RESULT (*openWritingPipeAddr)(struct rf24 *nrf24, const uint8_t* address);
     ...
     */
    WK_RESULT (*openReadingPipeAddr)(struct rf24 *nrf24, uint8_t number, const uint8_t* address);
     ...
     * @endcode
     */
    void (*printDetails)(struct rf24 *nrf24);
     ...
     */
    void (*printPrettyDetails)(struct rf24 *nrf24);
     ...
     * a `1` means the feature is enabled.
     */
    void (*sprintfPrettyDetails)(struct rf24 *nrf24, char *debugging_information);
     ...
     *   empty.
     */
    bool (*available)(struct rf24 *nrf24, uint8_t* pipe_num);
     ...
     *   empty or not.
     */
    bool (*rxFifoFull)(struct rf24 *nrf24);
     ...
     * @endcode
     */
    WK_RESULT (*powerDown)(struct rf24 *nrf24);
     ...
     */
    WK_RESULT (*powerUp)(struct rf24 *nrf24);
     ...
     * @endcode
     */
    WK_RESULT (*write_data)(struct rf24 *nrf24, const void* buf, uint8_t len, const bool multicast);
     ...
     * @endcode
     */
    WK_RESULT (*writeFast)(struct rf24 *nrf24, const void* buf, uint8_t len);
     ...
     * @endcode
     */
    WK_RESULT (*writeFast_data)(struct rf24 *nrf24, const void* buf, uint8_t len, const bool multicast);
     ...
     *   is on.
     */
    WK_RESULT (*writeBlocking)(struct rf24 *nrf24, const void* buf, uint8_t len, uint32_t timeout);
     ...
     *   is on.
     */
    bool (*txStandBy)(struct rf24 *nrf24);
     ...
     * @param startTx(false) If this is set to `true`, then this function puts the nRF24L01
     ...
     *   is on.
     */
    bool (*txStandByAfter)(struct rf24 *nrf24, uint32_t timeout, bool startTx);
     ...
     *   enableAckPayload().
     */
    bool (*writeAckPayload)(struct rf24 *nrf24, uint8_t pipe, const void* buf, uint8_t len);
     ...
     * @endcode
     */
    void (*whatHappened)(struct rf24 *nrf24, bool *tx_ok, bool *tx_fail, bool *rx_ready);
     ...
     * (true). Be sure to have called enableDynamicAck() at least once before
     * setting this parameter.
     * @param startTx(default=true) If this is set to `true`, then this function sets the
     ...
     * @endcode
     */
    WK_RESULT (*startFastWrite)(struct rf24 *nrf24, const void* buf, uint8_t len, const bool multicast, bool startTx);
     ...
     * @endcode
     */
    WK_RESULT (*startWrite)(struct rf24 *nrf24, const void* buf, uint8_t len, const bool multicast);
     ...
     */
    WK_RESULT (*reUseTX)(struct rf24 *nrf24);
     ...
     */
    WK_RESULT (*flush_tx)(struct rf24 *nrf24);
     ...
     * @return Current value of status register
     */
    WK_RESULT (*flush_rx)(struct rf24 *nrf24);
     ...
     */
    bool (*testCarrier)(struct rf24 *nrf24);
     ...
     * false if not.
     */
    bool (*testRPD)(struct rf24 *nrf24);
     ...
     */
    WK_RESULT (*isValid)(struct rf24 *nrf24);
     ...
     * is ignored.
     */
    WK_RESULT (*closeReadingPipe)(struct rf24 *nrf24, uint8_t pipe);
     ...
     * 5.
     */
    WK_RESULT (*setAddressWidth)(struct rf24 *nrf24, uint8_t a_width);
     ...
     */
    WK_RESULT (*setRetries)(struct rf24 *nrf24, uint8_t delay, uint8_t count);
     ...
     */
    WK_RESULT (*setChannel)(struct rf24 *nrf24, uint8_t channel);
     ...
     */
    uint8_t (*getChannel)(struct rf24 *nrf24);
     ...
     */
    WK_RESULT (*setPayloadSize)(struct rf24 *nrf24, uint8_t size);
     ...
     */
    uint8_t (*getPayloadSize)(struct rf24 *nrf24);
     ...
     */
    uint8_t (*getDynamicPayloadSize)(struct rf24 *nrf24);
     ...
     */
    void (*enableAckPayload)(struct rf24 *nrf24);
     ...
     */
    void (*disableAckPayload)(struct rf24 *nrf24);
     ...
     *
     */
    void (*enableDynamicPayloads)(struct rf24 *nrf24);
     ...
     *
     */
    void (*disableDynamicPayloads)(struct rf24 *nrf24);
     ...
     * @endcode
     */
    void (*enableDynamicAck)(struct rf24 *nrf24);
     ...
     * if its not.
     */
    bool (*isPVariant)(struct rf24 *nrf24);
     ...
     */
    void (*setAutoAck)(struct rf24 *nrf24, bool enable);
     ...
     */
    void (*setAutoAckPipe)(struct rf24 *nrf24, uint8_t pipe, bool enable);
     ...
     * @param lnaEnable(1) Enable or Disable the LNA (Low Noise Amplifier) Gain.
     ...
     */
    WK_RESULT (*setPALevel)(struct rf24 *nrf24, uint8_t level, bool lnaEnable);
     ...
     */
    uint8_t (*getPALevel)(struct rf24 *nrf24);
     ...
     */
    uint8_t (*getARC)(struct rf24 *nrf24);
     ...
     */
    bool (*setDataRate)(struct rf24 *nrf24, rf24_datarate_e speed);
     ...
     */
    rf24_datarate_e (*getDataRate)(struct rf24 *nrf24);
     ...
     */
    void (*setCRCLength)(struct rf24 *nrf24, rf24_crclength_e length);
     ...
     */
    rf24_crclength_e (*getCRCLength)(struct rf24 *nrf24);
     ...
     */
    void (*disableCRC)(struct rf24 *nrf24);
     ...
     */
    WK_RESULT (*maskIRQ)(struct rf24 *nrf24, bool tx_ok, bool tx_fail, bool rx_ready);
     ...
     * - disableCRC()
     */
    void (*startConstCarrier)(struct rf24 *nrf24, rf24_pa_dbm_e level, uint8_t channel);
     ...
     */
    void (*stopConstCarrier)(struct rf24 *nrf24);
     ...
     */
    void (*toggleAllPipes)(struct rf24 *nrf24, bool isEnabled);
     ...
     */
    WK_RESULT (*setRadiation)(struct rf24 *nrf24, uint8_t level, rf24_datarate_e speed, bool lnaEnable);
     ...
     */
    WK_RESULT (*openReadingPipe)(struct rf24 *nrf24, uint8_t number, uint64_t address);
     ...
     * @param address The 40-bit address of the pipe to open.
     */
    WK_RESULT (*openWritingPipe)(struct rf24 *nrf24, uint64_t address);
     ...
     */
    bool (*isAckPayloadAvailable)(struct rf24 *nrf24);
     ...
     * @return Current value of status register
     */
    uint8_t (*get_status)(struct rf24 *nrf24);
     ...
     * @warning Does nothing if stdout is not defined.  See fdevopen in stdio.h
     */
    void (*print_status)(struct rf24 *nrf24, uint8_t status);
     ...
     * @warning Does nothing if stdout is not defined.  See fdevopen in stdio.h
     */
    void (*print_observe_tx)(struct rf24 *nrf24, uint8_t value);
     ...
     * @param reg Which register. Use constants from nRF24L01.h
     * @param qty(1) How many successive registers to print
     */
    void (*print_byte_register)(struct rf24 *nrf24, const char* name, uint8_t reg, uint8_t qty);
     ...
     */
    void (*sprintf_byte_register)(struct rf24 *nrf24, char *out_buffer, uint8_t reg, uint8_t qty);
     ...
     * @param qty(1) How many successive registers to print
     */
    void (*print_address_register)(struct rf24 *nrf24, const char* name, uint8_t reg, uint8_t qty);
     ...
     * @param qty How many successive registers to print
     */
    void (*sprintf_address_register)(struct rf24 *nrf24, char *out_buffer, uint8_t reg, uint8_t qty);
     ...
     */
    WK_RESULT (*toggle_features)(struct rf24 *nrf24);
     ...
     */
    uint8_t (*_data_rate_reg_value)(struct rf24 *nrf24, rf24_datarate_e speed);
     ...
     */
    uint8_t (*_pa_level_reg_value)(struct rf24 *nrf24, uint8_t level, bool lnaEnable);

    /**@}*/

};

#endif /* end of include guard: _RF24_INTERFACE_H__ */
